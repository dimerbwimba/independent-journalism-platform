...
export async function generateMetadata({ params }: { params: Promise<{ location: string, country: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `${process.env.NEXT_PUBLIC_URL}/travel/${country}/${location}`
    }
  };
}
                ...
                <ul className="space-y-3">
                  {nearbyLocations.map(location => (
                    <li key={location.name}>
                      <Link
                        href={`/travel/hotels/${location.name?.toLowerCase().replace(/\s+/g, '-')}?country=${country}`}
          ...
          <script
            ...
            dangerouslySetInnerHTML={{
              __html: JSON.stringify({
                ...
                url: process.env.NEXT_PUBLIC_URL+`/travel/hotels/${location}?country=${country}`,
